...
public class Level
{
        ...
        for (int y = -factor; y <= factor; y++)
        {
            for (int x = -factor; x <= factor; x++)
            {
                if (y != 0 || x != 0)
                {
                    ...
                    var id = ids[environment.Random.Integer.UpTo(ids.Length)];
                    var spaceship = new Spaceship(
                        id: id,
                        position: 3000 * (Vector3.Forward * y + Vector3.Right * x) * factor * 2 / 5,
    ...
    public void UpdateScene(TimeSpan elapsedTime, LevelHandler.InputActions actions)
    {
        ...
        {
            var camera = Camera as SpaceshipFollowingCamera;
