    ...
    public void UpdateThrust(Vector3 Position, Vector3 Direction, Vector3 Up, TimeSpan ElapsedTime, Environment Environment)
    {
        if (LastPosition.HasValue && LastDirection.HasValue)
        {
            ...
            if (count > 4)
            {
                ...
                {
                    var value = (float)(i + Environment.Random.Real.Unsigned) / (float)count;

                    var flame = new Flame()
                    {
                        Position = Vector3.Hermite(LastPosition.Value, LastDirection.Value, Position, Direction, value) + Environment.Random.Vector3D.UnitPoint * 0.1f,
                        Up = Up,
                        Direction = -Direction
                    };
