...
public class LevelHandler : Scenes.Handler<Scenes.Level>
{
    ...
    class Fader
    {
        ...
        public bool Fading => Value != to;

        private float from;
        private float to;
        private TimeSpan fadeTime;
        private TimeSpan faded;
        public static Fader Init(float value) => new() { Value = value };
        public void BeginFade(float to, TimeSpan fadeTime)
        {
            this.from = Value;
            this.to = to;
            this.fadeTime = fadeTime;
            this.faded = TimeSpan.Zero;
        }
        public void Update(TimeSpan timeSpan)
        {
            ...
            if (faded < fadeTime)
            {
                Value = (float)(to * (faded.TotalSeconds / fadeTime.TotalSeconds) + from * (1 - faded.TotalSeconds / fadeTime.TotalSeconds));
            }
            else
            {
                Value = to;
    ...
    public LevelHandler(Scenes.Level scene)
    {
        ...
        var fadeIn = Fader.Init(0);
        var fadeOut = Fader.Init(0);
        ...
        var wireframe = false;
        var eye = default(RenderTarget2D);
        ...
        Begin = async content =>
        {
            ...
            await Task.Run(() =>
            {
                ...
                models = content.GetAll<Model>(scene.Environment.Models);
                environment.Sounds = content.GetAll<SoundEffect>(scene.Environment.Sounds);
                textures = content.GetAll<Texture2D>(scene.Environment.Textures);
            ...
            fadeIn.BeginFade(1f, TimeSpan.FromSeconds(.5f));
        };
        ...
        Update = (timeSpan, input) =>
        {
            ...
            fadeIn.Update(timeSpan);
            fadeOut.Update(timeSpan);

            level.UpdateScene(
                fadeIn.Fading ? TimeSpan.Zero : timeSpan,
                actions
            );
            ...
            if (won.HasChanged || dead.HasChanged)
            {
                fadeOut.BeginFade(1, TimeSpan.FromSeconds(3));
            }
        ...
        Draw = (graphics, started) =>
        {
            ...
            if (started)
            {
                ...
                var camera = new CameraModel(level.Camera, 0f, new System.Drawing.Size(g.Viewport.Width, g.Viewport.Height));
                ...
                void DrawText()
                {
                    ...
                    var playerVerticalRotation = $"vertical Rotation: {level.LocalPlayer.ControlledObject.Pitch}";
                    var playerVerticalRotationLength = spriteFonts[Data.Font].MeasureString(playerVerticalRotation).X;

                    var osdBlend = Microsoft.Xna.Framework.Color.White * (1f - MathHelper.Clamp(fadeOut.Value * 1.5f - 1, 0, 1));
                    ...
                    if (fadeOut.Value != 0)
                    {
                        s.Draw(textures[Data.BlackBackground], g.Viewport.Bounds, new Microsoft.Xna.Framework.Color(0, 0, 0, fadeOut.Value / 2));
                ...
                void DrawCameraEffects()
                {
                    ...
                    if (fadeIn.Value < 1)
                    {
                        s.Draw(textures[Data.BlackBackground], g.Viewport.Bounds, new Microsoft.Xna.Framework.Color(1f, 1f, 1f, 1f - fadeIn.Value));
                    }
                    s.End();
                }
                DrawCameraEffects();
            ...
            s.Begin();
            s.Draw(eye, g.Viewport.Bounds, Microsoft.Xna.Framework.Color.White);
            s.End();
        };
    }
}